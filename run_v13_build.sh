...
"$PYTHON_BIN" scripts/audit_endpoint_sources.py \
  --output data/processed_v13/audit \
  --papp-decision-registry results/analysis/papp_provenance_decisions_v2.csv \
  --papp-unresolved-policy exclude \
  --human-evidence-registry results/analysis/human_evidence_decisions_v11.csv

"$PYTHON_BIN" scripts/rebuild_endpoint_datasets.py \
  --audit-dir data/processed_v13/audit \
  --output data/processed_v13/datasets

"$PYTHON_BIN" scripts/build_split_manifest.py \
  --datasets-dir data/processed_v13/datasets \
  --cache-path data/processed_v13/_cache/structure_groups.sqlite \
  --output data/processed_v13/splits

"$PYTHON_BIN" scripts/verify_dataset_version.py \
  --baseline-dir data/processed_v12 \
  --candidate-dir data/processed_v13 \
  --expect-delta Thalf__human__terminal_iv:train:-1:-1 \
  --expect-delta Thalf__human__terminal_iv:val:0:0 \
...
"$PYTHON_BIN" scripts/audit_human_evidence_expansion.py \
  --audit-dir data/processed_v13/audit \
  --output results/analysis/human_evidence_expansion_v13
