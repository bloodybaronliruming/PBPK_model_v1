#!/usr/bin/env python3
"""Lock B1 eligibility and strict-F non-overlap for public-F source leads.

The stage stores only source semantics; B2 numerical extraction remains blocked
until source-specific full-text locators and policies are registered.
"""
...
def lock(queue: pd.DataFrame, decisions: pd.DataFrame, registry: pd.DataFrame, candidate_ids: set[str]) -> pd.DataFrame:
    required_queue = {"doc_id", "doi", "pubmed_id", "title", "bibliographic_route", "prior_definition_audit_status"}
    required_registry = {"doc_id", "parent_id", "scaffold_group", "public_status"}
    if not required_queue <= set(queue.columns) or not required_registry <= set(registry.columns):
        raise ValueError("Prefilter queue or candidate registry schema mismatch")
    ...
    candidate = registry.loc[registry.doc_id.astype(str).isin(candidate_ids)].copy()
    candidate["doc_id"] = candidate.doc_id.astype(str)
    if set(candidate.doc_id) != candidate_ids:
        raise ValueError("Requested eligibility source is absent from public-F candidate registry")
    if not candidate.public_status.eq("public_candidate_definition_audit").all():
        raise ValueError("A strict-F-quarantined source cannot use this unquarantined eligibility lock")
    isolation = candidate.groupby("doc_id", as_index=False).agg(
        candidate_records=("doc_id", "size"),
        parent_ids=("parent_id", lambda values: ";".join(sorted(set(values)))),
    ...
    isolation["strict_f_isolation_disposition"] = "nonoverlap_public_candidate_pending_B2"
    result = scope.merge(decisions, on="doc_id", how="inner", validate="one_to_one")
    return result.merge(isolation, on="doc_id", how="inner", validate="one_to_one").sort_values("doc_id", ignore_index=True)


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--prefilter", type=Path,
                        default=ROOT / "results/analysis/public_f_all_source_bibliographic_prefilter_v8")
    parser.add_argument("--registry", type=Path,
                        default=ROOT / "results/analysis/public_f_development_candidate_registry_v1")
    parser.add_argument("--decisions", type=Path,
                        default=ROOT / "data/manual_review/public_f_unquarantined_priority_eligibility_decisions_v1.csv")
    parser.add_argument("--doc-ids", default="98922,98935")
    ...
    result = lock(pd.read_csv(queue_file, dtype=str, keep_default_na=False), read_decisions(args.decisions),
                  pd.read_csv(registry_file, dtype=str, keep_default_na=False), candidate_ids)
    summary = {
        "selected_documents": len(result),
        "source_clusters": int(result.doi.nunique()),
        "b1_qualified_documents": len(result),
        "nonoverlap_public_candidate_documents": len(result),
        "b2_numeric_lock_authorized": False,
        "b2_block_reason": "fulltext_table_or_results_locator_and_condition_policy_required",
        "numeric_values_read": False,
        "strict_f_modified": False,
        "public_f_training_authorized": False,
    }
    if args.check_only:
        print(f"Public-F unquarantined priority eligibility lock valid: documents={len(result)} B1={len(result)}")
        return
    with stage_output(args.output) as out:
        result.to_csv(out / "source_eligibility_lock_blinded.csv", index=False)
        (out / "summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        (out / "README.md").write_text(
            "# Public-F unquarantined priority source eligibility lock\n\n"
            "Both sources are B1-qualified primary human oral+IV parent-analyte studies with directly reported absolute-F evidence. "
            "They are non-overlapping public candidates, not additions to strict-F. Numerical extraction remains blocked until each source "
            "has a full-text table or results locator and a registered condition/matrix policy.\n",
            encoding="utf-8",
        )
        finish_stage(out, "public_f_unquarantined_priority_source_eligibility_lock", inputs={
    ...
    print(f"Public-F unquarantined priority eligibility lock: {args.output}")
