...
"$PYTHON_BIN" scripts/audit_endpoint_sources.py \
  --output data/processed_v10/audit \
  --papp-decision-registry results/analysis/papp_provenance_decisions_v2.csv \
  --papp-unresolved-policy exclude \
  --human-evidence-registry results/analysis/human_evidence_decisions_v6.csv

"$PYTHON_BIN" scripts/rebuild_endpoint_datasets.py \
  --audit-dir data/processed_v10/audit \
  --output data/processed_v10/datasets

"$PYTHON_BIN" scripts/build_split_manifest.py \
  --datasets-dir data/processed_v10/datasets \
  --cache-path data/processed_v10/_cache/structure_groups.sqlite \
  --output data/processed_v10/splits

"$PYTHON_BIN" scripts/verify_dataset_version.py \
  --baseline-dir data/processed_v9 \
  --candidate-dir data/processed_v10 \
...
"$PYTHON_BIN" scripts/audit_human_evidence_expansion.py \
  --audit-dir data/processed_v10/audit \
  --output results/analysis/human_evidence_expansion_v10
