...
then
  echo "CUDA is not visible in the oneadmet environment; formal shared/private training was not started." >&2
  exit 1
fi
conda run --no-capture-output -n oneadmet \
  python scripts/run_multitask_shared_private_traincv.py \
  --device cuda \
