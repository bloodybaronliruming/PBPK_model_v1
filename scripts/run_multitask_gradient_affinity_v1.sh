...
then
  echo "CUDA is not visible in the oneadmet environment; formal gradient-affinity diagnostic was not started." >&2
  exit 1
fi
conda run --no-capture-output -n oneadmet \
  python scripts/analyze_multitask_gradient_affinity.py \
  --device cuda \
