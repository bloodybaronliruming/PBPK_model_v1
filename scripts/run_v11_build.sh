...
"$PYTHON_BIN" scripts/audit_endpoint_sources.py \
  --output data/processed_v11/audit \
  --papp-decision-registry results/analysis/papp_provenance_decisions_v2.csv \
  --papp-unresolved-policy exclude \
  --human-evidence-registry results/analysis/human_evidence_decisions_v8.csv

"$PYTHON_BIN" scripts/rebuild_endpoint_datasets.py \
  --audit-dir data/processed_v11/audit \
  --output data/processed_v11/datasets

"$PYTHON_BIN" scripts/build_split_manifest.py \
  --datasets-dir data/processed_v11/datasets \
  --cache-path data/processed_v11/_cache/structure_groups.sqlite \
  --output data/processed_v11/splits

"$PYTHON_BIN" scripts/verify_dataset_version.py \
  --baseline-dir data/processed_v10 \
  --candidate-dir data/processed_v11 \
  --expect-delta Thalf__human__terminal_iv:train:0:0 \
  --expect-delta Thalf__human__terminal_iv:val:-2:0 \
  --expect-delta Thalf__human__terminal_iv:test:0:0 \
  --expect-delta F__human__absolute_oral:train:1:0 \
...
"$PYTHON_BIN" scripts/audit_human_evidence_expansion.py \
  --audit-dir data/processed_v11/audit \
  --output results/analysis/human_evidence_expansion_v11
