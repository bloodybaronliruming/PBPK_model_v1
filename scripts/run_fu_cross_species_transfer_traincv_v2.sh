...
# Frozen plasma-fu physical-scale comparison: 5 outer folds x 3 seeds x 3 routes.
...
then
  echo "CUDA is not visible in the oneadmet environment; formal fu training was not started." >&2
  exit 1
fi
conda run --no-capture-output -n oneadmet \
  python scripts/run_cross_species_transfer_traincv.py \
  --protocol data/public_development/fu_cross_species_transfer_protocol_v2 \
  --device cuda \
